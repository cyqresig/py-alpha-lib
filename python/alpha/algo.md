...
- QUANTILE(input: np.ndarray[float], periods: int, quantile: float): Rolling Quantile (Percentile)  Calculates the quantile value over a rolling window of `periods` elements. `quantile` should be in range [0.0, 1.0], where 0.0 is the minimum and 1.0 is the maximum.  Uses linear interpolation between adjacent ranks (same as numpy/pandas default).  This operator is used to compute Alpha158's QTLU/QTLD factors: QTLU = Quantile(close, N, 0.8) QTLD = Quantile(close, N, 0.2)
